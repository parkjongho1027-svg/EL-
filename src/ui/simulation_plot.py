...
def draw_scurve_plot(canvas, profile, palette):
    ...
    trace("속도 (m/s)", speed, (0.0, max(speed) * 1.05), *upper, speed_color)
    lower_limit = min(0.0, min(powers))
    upper_limit = max(0.0, max(powers))
    if lower_limit == upper_limit:
        upper_limit = 1.0
    trace(
        "추정 계통전력 (kW, +소비 / −회수)"
        if electrical
        else "예상 기계동력 (kW, +구동 / −제동)",
        powers,
...
def draw_energy_comparison(canvas, reference, candidate, palette):
    ...
    if not reference or not candidate:
        return
    width = max(320, canvas.winfo_width())
    height = max(175, canvas.winfo_height())
    ...
    for key, label, top, bottom in (
        ("speed", "속도 m/s", 32, 75),
        ("power", "계통전력 kW (+소비 / −회수)", 97, height - 24),
    ):
        series = []
        for profile in (reference, candidate):
            values = (
                [sample[2] for sample in profile["samples"]]
                if key == "speed"
                else profile["grid_kw_samples"]
            )
            series.append(values)
        minimum = min(0.0, *(min(v) for v in series))
        maximum = max(0.0, *(max(v) for v in series))
        if maximum - minimum < 1e-12:
            maximum = minimum + 1
        canvas.create_text(
            left, top - 9, text=label, anchor="w", fill=ink, font=("맑은 고딕", 8)
        )
        ...
        for profile, values, color in zip((reference, candidate), series, colors):
            ...
            for i in indices:
                coords.extend(
                    (
                        left + profile["samples"][i][0] / duration * (right - left),
                        bottom
                        - (values[i] - minimum) / (maximum - minimum) * (bottom - top),
                    )
                )
            if len(coords) >= 4:
...
def draw_scurve_plot(canvas, profile, palette):
    ...
    try:
        from src.ui.plot_render import draw_on_canvas
...
def _native_draw(canvas, profile, palette, reference=None):
    ...
    try:
        ...
        if reference:
            return _legacy_draw_energy_comparison(canvas, reference, profile, palette)
        return _legacy_draw_scurve_plot(canvas, profile, palette)
