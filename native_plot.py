...
def _rgb(value):
    if not isinstance(value,str) or len(value)!=7 or value[0]!='#':return (40,40,40)
...
def render_png(profile,palette,reference=None,size=(1100,440),axes=None):
    ...
    bg=_rgb(palette.get('surface','#ffffff'));ink=_rgb(palette.get('text','#222222'))
    ...
    def dot(x,y,color,thickness=1):
        ...
        for dy in range(-(thickness//2),thickness-thickness//2):
            ...
            for dx in range(-(thickness//2),thickness-thickness//2):
                xx=x+dx
                if 0<=xx<w:
                    i=(yy*w+xx)*3;pixels[i:i+3]=bytes(color)
    def line(a,b,color,thickness=1):
        x0,y0=a;x1,y1=b
        steps=max(1,int(max(abs(x1-x0),abs(y1-y0))))
        for j in range(steps+1):
            t=j/steps;dot(x0+(x1-x0)*t,y0+(y1-y0)*t,color,thickness)
                ...
                for col,pixel in enumerate(bits):
                    if pixel=='1':
                        for dx in range(mult):
                            for dy in range(mult):dot(x+col*mult+dx,y+row*mult+dy,color)
            x+=6*mult
    def step(raw):
        unit=10**math.floor(math.log10(max(raw,1e-12)))
        for n in (1,2,2.5,5,10):
            if n*unit>=raw:return n*unit
        return 10*unit
    ...
    power_top=max(speed_bottom+35,round(h*.62))
    for key,top,bottom,title in (
        ('speed',50,speed_bottom,'SPEED (M/S)'),
        ('power',power_top,h-32,'GRID POWER (KW)' if electrical else 'MECHANICAL POWER (KW)')):
        values=[]
        for _,p,_ in profiles:
            values.append([row[2] for row in p['samples']] if key=='speed' else p['grid_kw_samples'] if electrical else p['signed_mechanical_kw_samples'])
        lo=min(0.,*(min(v) for v in values));hi=max(0.,*(max(v) for v in values))
        ...
        for i in range(round(tmax/ts)+1):
            t=i*ts;x=left+(right-left)*t/tmax
            line((x,top),(x,bottom),grid)
            if key=='power':
                num=f'{t:g}';label(num,x-len(num)*6,bottom+3,mult=2)
        for (_,p,color),vlist in zip(profiles,values):
            previous=None
            for sample,value in plot_points(p['samples'],vlist):
                point=(left+(right-left)*sample[0]/tmax,bottom-(bottom-top)*(value-lo)/(hi-lo))
                if previous is not None:line(previous,point,color,2)
                previous=point
