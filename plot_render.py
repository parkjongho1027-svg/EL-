...
def render_plot(profile, palette, reference=None, size=(820,270), scale=2):
    ...
    width,height=size
    if width<360 or height<200:raise ValueError('그래프 크기는 360×200 이상이어야 합니다.')
    ...
    f=_font(10*scale);title_font=_font(11*scale)
    left,right=75*scale,(width-20)*scale
    profiles=(('Reference',reference,'#4d9aff'),('Candidate',profile,'#f3a450')) if reference else (('Curve',profile,palette.get('accent','#2879cc')),)
    duration=max(p['duration_s'] for _,p,_ in profiles)
    if duration<=0:raise ValueError('운행시간이 0보다 커야 합니다.')
    def nice_step(raw):
        if raw<=0:return 1.
        power=10**math.floor(math.log10(raw))
        for number in (1,2,2.5,5,10):
            if number*power>=raw:return number*power
        return 10*power
    ...
    def series(p,key):
        if key=='speed':return [sample[2] for sample in p['samples']]
        return p['grid_kw_samples'] if electrical else p['signed_mechanical_kw_samples']
    speed_bottom=round(height*.46)
    power_top=max(speed_bottom+35,round(height*.62))
    bands=((50*scale,speed_bottom*scale,'Speed (m/s)','speed'),
           (power_top*scale,(height-32)*scale,'Grid power (kW)' if electrical else 'Mechanical power (kW)','power'))
    for top,bottom,label,key in bands:
        arrays=[series(p,key) for _,p,_ in profiles]
        observed_low=min(0.,*(min(a) for a in arrays))
        observed_high=max(0.,*(max(a) for a in arrays))
        if observed_high-observed_low<1e-9:observed_high=observed_low+1
        step=nice_step((observed_high-observed_low)/2)
        low=0. if key=='speed' else math.floor((observed_low-step*0.05)/step)*step
        ...
        for j in range(tick_count):
            ...
            d.text((left-7*scale-(box[2]-box[0]),y-(box[3]-box[1])/2-2*scale),txt,fill=text,font=f)
        for j in range(round(time_max/time_step)+1):
            ...
            if key=='power':
                txt=f'{tick:g}'
                box=d.textbbox((0,0),txt,font=f)
                d.text((x-(box[2]-box[0])/2,bottom+3*scale),txt,fill=text,font=f)
        for name,p,color in profiles:
            ...
            coords=[(left+(right-left)*p['samples'][i][0]/time_max,
                     bottom-(bottom-top)*(values[i]-low)/(high-low)) for i in indices]
            if len(coords)>1:d.line(coords,fill=color,width=2*scale,joint='curve')
    ...
    if reference:
        for index,(name,_,color) in enumerate(profiles):
            ...
            d.line((x,8*scale,x+18*scale,8*scale),fill=color,width=3*scale)
...
def draw_on_canvas(canvas,profile,palette,reference=None):
    canvas.delete('all')
    if not profile or (reference is None and not profile):return
    ...
    photo=ImageTk.PhotoImage(image.resize((width,height),Image.Resampling.LANCZOS),master=canvas)
