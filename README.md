...
공학 데이터 도구의 승강로 위치 모델은 전체 층수·출발층·도착층을 선택하고 상승 또는 하강 이동을 재생합니다. 재생과 슬라이더 이동 시 출발·가속·주행·감속·도착 단계를 표시합니다. 기계동력 화면에 입력한 거리를 1층에서 최상층까지의 **전체 행정**으로 간주하고 모든 층고가 같다는 가정으로 해당 구간을 다시 계산합니다. 카와 균형추의 화면 크기는 식별을 돕기 위한 도식이며 실제 치수나 승강로 설계 검증 자료가 아닙니다.
