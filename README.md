...
전동기 상세창은 입력 중 350ms 지연 계산, Enter 수동 계산·입력 기록, 입력칸 위 휠 스크롤, 이전 입력·결과 복사를 지원합니다. 입력 기록창은 프로젝트 관리창처럼 행 클릭/드래그 체크, 선택 삭제, 기록명 변경을 지원합니다. 로프 장력·층별 5분 수송 그래프는 별도 창에서 고정·PNG 저장을 지원합니다. 그래프명과 담당자, 입력 상태를 저장하고 종류별 그래프 관리에서 여러 개를 선택해 불러오거나 삭제할 수 있습니다. **기계동력 기록을 여러 개 선택하면 한 창의 공통 축에 다른 색 곡선으로 겹쳐 표시합니다. 전기에너지 기록을 여러 개 선택하면 한 창에 공통 축을 적용한 그래프를 세로로 나열하고 스크롤합니다.** 비교 화면은 최대 12개를 지원하며, 개별 그래프는 기존 PNG 저장을 지원합니다. 입력 조건을 다시 계산하므로 PNG 이미지를 기록에 내장하지 않습니다. 수송 그래프는 먼저 교통량 계산이 끝나야 열리며 그래프 저장은 수송 그래프창에서 실행합니다. 시뮬레이션 탭의 그래프는 화면에서 자동 갱신하고, 저장된 기계동력·전기에너지 그래프는 각 탭의 ‘시뮬레이션 관리’에서 별도로 불러옵니다.

공학 데이터 도구의 승강로 위치 모델은 전체 층수와 도착층을 선택하거나 오른쪽 층 번호를 클릭하여 상승·하강 이동을 시작합니다. 도착 후 출발층은 해당 도착층으로 자동 갱신합니다. 운행 중 다른 목적층을 선택하면 **현재 카 위치에서 가장 가까운 층에 정차한 것으로 단순화**한 뒤 새 운행을 시작합니다. 오른쪽 승강로 도식은 층당 동일한 화면 간격을 유지하며 세로 스크롤합니다. 카 **바닥**이 해당 층 눈금에 맞고, 왼쪽 현재 상태 상자에는 재생·슬라이더 이동에 따라 출발·가속·주행·감속·도착을 표시합니다. 기계동력 화면에 입력한 거리를 1층에서 최상층까지의 **전체 행정**으로 간주하고 모든 층고가 같다는 가정으로 해당 구간을 다시 계산합니다. 카와 균형추의 화면 크기는 식별을 돕기 위한 도식이며 실제 치수나 승강로 설계 검증 자료가 아닙니다.
