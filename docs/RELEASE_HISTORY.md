...
현재 작업 트리 빌드 `2026.09.25.82`는 공개 `v81.0.0` 태그의 후속 변경입니다. 승강로 도식의 층고 화면 간격·카 바닥 정렬, 도착층에서 다음 출발층 자동 갱신, 층 번호 선택 및 왼쪽 운행 상태 영역을 포함합니다. 이번 변경의 게시 여부는 새 Git 커밋과 원격 상태로 확인해야 합니다.
